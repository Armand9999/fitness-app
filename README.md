...
After changing the database schema, refresh the checked-in TypeScript database contract:

```bash
supabase gen types typescript --local > app/lib/database.types.ts
```

The checked-in database types intentionally include more specific JSON-column shapes used by the current application. Review regenerated JSON fields before replacing the file, and keep application validation aligned with the database contract.
